    ...
    , ui(new Ui::Widget)
{
    ...
    /*qDebug() << "OPENVIDEO value: " << OPENVIDEO;
    qDebug() << "ui is not null: " << static_cast<bool>(ui);
    qDebug() << "openVideo is not null: " << static_cast<bool>(ui->openVideo);*/
    ...
    _sendImg->moveToThread(_imgThread); // 新启线程接受视频帧
    _sendImg->start();
    //_imgThread->start();
    // 处理每一帧数据
...
void Widget::on_exitmeetBtn_clicked()
{
    ...
    QMessageBox::warning(this, "Information", QStringLiteral("退出会议"), QMessageBox::Yes, QMessageBox::Yes);
...
void Widget::on_openVideo_clicked()
{
    if (_camera->status() == QCamera::ActiveStatus)
    {
        ...
        if (_camera->error() == QCamera::NoError)
        {
            _imgThread->quit();
            _imgThread->wait();
            ui->openVideo->setText(QStringLiteral("开启摄像头"));
    ...
    else
    {
        ...
        {
            _imgThread->start();
            ui->openVideo->setText(QStringLiteral("关闭摄像头"));
        }
    }
}
...
void Widget::on_connServer_clicked()
{
    ...
    {
        QMessageBox::warning(this, "Input Error", QStringLiteral("Ip Error"), QMessageBox::Yes, QMessageBox::Yes);
        return;
    }
    if (portvalidate.validate(port, pos) != QValidator::Acceptable)
    {
        QMessageBox::warning(this, "Input Error", QStringLiteral("Port Error"), QMessageBox::Yes, QMessageBox::Yes);
        return;
    }

    if (_mytcpSocket->connectToServer(ip, port, QIODevice::ReadWrite))
    {
        ui->outlog->setText(QStringLiteral("成功连接到") + ip + ":" + port);
        ui->openAudio->setDisabled(true);
        ui->openVideo->setDisabled(true);
        ui->createmeetBtn->setDisabled(false);
        ui->exitmeetBtn->setDisabled(true);
        ui->joinmeetBtn->setDisabled(false);
        WRITE_LOG("succeed connecting to %s:%s", ip.toStdString().c_str(), port.toStdString().c_str());
        QMessageBox::warning(this, "Connection success", QStringLiteral("成功连接服务器"), QMessageBox::Yes, QMessageBox::Yes);
        ui->sendmsg->setDisabled(false);
        ui->connServer->setDisabled(true);
    }
    else
    {
        ui->outlog->setText(QStringLiteral("连接失败,请重新连接..."));
        WRITE_LOG("failed to connect %s:%s", ip.toStdString().c_str(), port.toStdString().c_str());
...
void Widget::datasolve(MESG* msg)
{
    if (msg->msg_type == CREATE_MEETING_RESPONSE)
    {
        ...
        {
            _createmeet = false;
            QMessageBox::information(this, "Room Information", QStringLiteral("无可用房间"), QMessageBox::Yes, QMessageBox::Yes);
    ...
    else if (msg->msg_type == JOIN_MEETING_RESPONSE)
    {
        ...
        {
            QMessageBox::warning(this, "Meeting information", QStringLiteral("成员已满，无法加入"), QMessageBox::Yes, QMessageBox::Yes);
            ui->outlog->setText(QString("成员已满，无法加入"));
            WRITE_LOG("full room, cannot join");
        }
        else if (c > 0)
        {
            QMessageBox::warning(this, "Meeting information", QStringLiteral("加入成功"), QMessageBox::Yes, QMessageBox::Yes);
    ...
    else if (msg->msg_type == RemoteHostClosedError)
    {
        ...
        _mytcpSocket->disconnectFromHost();
        _mytcpSocket->wait();
        ui->outlog->setText(QString("关闭服务器的连接"));
        ui->createmeetBtn->setDisabled(true);
        ui->exitmeetBtn->setDisabled(true);
        ui->connServer->setDisabled(false);
        ui->joinmeetBtn->setDisabled(true);
        ...
        if (_createmeet || _joinmeet) 
            QMessageBox::warning(this, "Meeting Information", QStringLiteral("会议结束"), QMessageBox::Yes, QMessageBox::Yes);
    }
    else if (msg->msg_type == OtherNetError)
    {
        QMessageBox::warning(NULL, "Network Error", QStringLiteral("网络异常"), QMessageBox::Yes, QMessageBox::Yes);
        clearPartner();
        _mytcpSocket->disconnectFromHost();
        _mytcpSocket->wait();
...
Partner* Widget::addPartner(quint32 ip)
{
    ...
    else
    {
        ...
        // 当有人加入时，开启滑动条滑动事件，开启输入（只有自己时，打不开）
        if (partner.size() > 1)
        {
...
void Widget::on_joinmeetBtn_clicked()
{
    ...
    {
        QMessageBox::warning(this, "RoomNo Error", QStringLiteral("房间号不合法"), QMessageBox::Yes, QMessageBox::Yes);
    }
    else
    {
        // 加入发送队列
