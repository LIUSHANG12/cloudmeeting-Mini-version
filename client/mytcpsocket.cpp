...
MyTcpSocket::MyTcpSocket(QObject *par)
	: QThread(par)
{
...
void MyTcpSocket::sendData(MESG* send)
{
	...
	//创建会议,发送音频,关闭摄像头，发送图片
	if (send->msg_type == CREATE_MEETING || send->msg_type == AUDIO_SEND || send->msg_type == CLOSE_CAMERA
		|| send->msg_type == IMG_SEND || send->msg_type == TEXT_SEND)
	{
		// 发送数据大小
...
void MyTcpSocket::recvFromSocket()
{
	...
	*/
	...
	else
	{
		...
		{
			if (recvbuf[0] == '$' && recvbuf[MSG_HEADER + data_size] == '#') //收到的包结构正确
			{
				MSG_TYPE msgtype;
				uint16_t type;
